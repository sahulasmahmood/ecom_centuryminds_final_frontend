    ...
    try {
      ...
      if (response.success) {
        ...
        setFormData({
          ...
          // Compliance fields
          isGreenCracker: product.isGreenCracker ?? true,
          greenCrackerType: product.greenCrackerType || "SWAS",
          bariumFree: product.bariumFree ?? true,
          pesoLicenseNo: product.pesoLicenseNo || "",
          batchNumber: product.batchNumber || "",
          manufacturingDate:
            product.manufacturingDate || new Date().toISOString().split("T")[0],
          contentWeight: product.contentWeight || "",
          noiseLevel: product.noiseLevel || "",
          ageRestriction: product.ageRestriction ?? true,
          safetyInstructions:
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      Object.entries(formData).forEach(([key, value]) => {
        if (key === "image" || key === "currentImageUrl") return;
      ...
      if (updateData.gstPercentage)
        updateData.gstPercentage = parseFloat(updateData.gstPercentage);
    ...
    <div className="min-h-screen bg-gray-50/50 pb-20">
      ...
      <div className="container mx-auto px-6 py-8">
        <p className="text-sm text-gray-500 mb-6">
          Product ID: <span className="font-mono font-medium">{productId}</span>
        </p>
